...
std::vector<Point> CreateSpiralMicrophones(int count,
                                           float base_radius,
                                           int spirals,
                                           float pitch) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        Point microphone_pos = { cos(spiral_num * tau/spirals + tau/count*i/spirals) * mic_radius,
                                 sin(spiral_num * tau/spirals + tau/count*i/spirals) * mic_radius,
                                 0 };
...
int main(int argc, char *argv[]) {
    ...
    for (const Point &micro_pos : micro_positions) {
        ...
        MicrophoneSamples *recording = new MicrophoneSamples(micro_pos,
                                                             sample_count);
    ...
    canvas.SetPixel((s.x/range + 0.5) * resolution_pixels,
                    ((0-s.y)/range + 0.5) * resolution_pixels,
                    255, 255, 255);
    s = sound_source2;
    s.MakeUnitLen();
    canvas.SetPixel((s.x/range + 0.5) * resolution_pixels,
